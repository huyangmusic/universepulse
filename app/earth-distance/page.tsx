...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: messages.earthDistance.hero.title,
    description: messages.earthDistance.hero.description,
    alternates: {
      canonical: `${BASE}/earth-distance`,
      languages: {
        'en': '/earth-distance',
        'zh': '/zh/earth-distance',
        'ja': '/ja/earth-distance',
        'es': '/es/earth-distance',
        'ar': '/ar/earth-distance',
        'fr': '/fr/earth-distance',
    ...
    openGraph: {
      ...
      url: `${BASE}/earth-distance`,
      siteName: 'UniversePulse',
      title: messages.earthDistance.hero.title,
      description: messages.earthDistance.hero.description,
      images: [{ url: `${BASE}/api/og?theme=earth-distance`, width: 1200, height: 630, alt: 'UniversePulse — Earth Distance' }],
    },
    twitter: {
      card: 'summary_large_image',
      title: messages.earthDistance.hero.title,
      description: messages.earthDistance.hero.description,
      images: [`${BASE}/api/og?theme=earth-distance`],
    },
    robots: {
      index: true,
      follow: true,
...
export default async function EarthDistancePage() {
  const locale = await detectLocale();
  const messages = await getMessages({ locale });

  const faqItems = messages.earthDistance.faq.items as { q: string; a: string }[];
      ...
      <div className="min-h-screen bg-background text-text-primary">
        ...
        <div className="max-w-4xl mx-auto px-6 py-20">
          <div className="text-center mb-16">
            <h1 className="text-4xl md:text-6xl font-bold mb-6 font-syne bg-gradient-to-r from-primary via-purple-400 to-accent-purple bg-clip-text text-transparent">
              {messages.earthDistance.hero.title}
            </h1>
            <p className="text-xl text-text-secondary max-w-2xl mx-auto leading-relaxed">
              {messages.earthDistance.hero.description}
            </p>
          </div>

          <div className="bg-surface/50 backdrop-blur-xl border border-border/50 rounded-2xl p-8 md:p-12 text-center mb-16">
            <h2 className="text-2xl font-semibold mb-4">{messages.earthDistance.cta.title}</h2>
            <p className="text-text-secondary mb-8">{messages.earthDistance.cta.description}</p>
            <Link
              href="/"
            ...
            >
              {messages.earthDistance.cta.button}
          ...
          <div className="space-y-12">
            <section>
              <h2 className="text-2xl md:text-3xl font-semibold mb-6 font-syne text-primary">
                {messages.earthDistance.cosmos.title}
              </h2>
              <div className="bg-surface/30 border border-border/30 rounded-xl p-8">
                <p className="text-text-secondary leading-relaxed">{messages.earthDistance.cosmos.description}</p>
              </div>
            </section>

            <section>
              <h2 className="text-2xl md:text-3xl font-semibold mb-6 font-syne text-primary">
                {messages.earthDistance.faq.title}
              </h2>
          ...
          <section className="mt-16">
            <h2 className="text-2xl md:text-3xl font-semibold mb-6 font-syne text-primary">
              {messages.earthDistance.explore.title}
            </h2>
              ...
              <Link href="/born-since" className="group">
                <div className="bg-surface/30 border border-border/30 rounded-xl p-6 hover:border-primary/40 transition-colors duration-200 h-full">
                  ...
                  <h3 className="text-lg font-semibold text-text-primary mb-2 group-hover:text-primary transition-colors">
                    {messages.earthDistance.explore.born.title}
                  </h3>
                  <p className="text-sm text-text-secondary">{messages.earthDistance.explore.born.desc}</p>
                </div>
              </Link>
              <Link href="/co2-since" className="group">
                <div className="bg-surface/30 border border-border/30 rounded-xl p-6 hover:border-red-400/40 transition-colors duration-200 h-full">
                  ...
                  <h3 className="text-lg font-semibold text-text-primary mb-2 group-hover:text-red-400 transition-colors">
                    {messages.earthDistance.explore.co2.title}
                  </h3>
                  <p className="text-sm text-text-secondary">{messages.earthDistance.explore.co2.desc}</p>
                </div>
              </Link>
              <Link href="/sea-level-rise" className="group">
                <div className="bg-surface/30 border border-border/30 rounded-xl p-6 hover:border-blue-400/40 transition-colors duration-200 h-full">
                  ...
                  <h3 className="text-lg font-semibold text-text-primary mb-2 group-hover:text-blue-400 transition-colors">
                    {messages.earthDistance.explore.sea.title}
                  </h3>
                  <p className="text-sm text-text-secondary">{messages.earthDistance.explore.sea.desc}</p>
                </div>
              </Link>
            </div>
          </section>
