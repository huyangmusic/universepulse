...
* **动态 OG 图片**：社交分享时展示实时数据预览，Vercel Edge Function (`app/api/og/route.tsx`) + Satori，支持 4 种主题色
...
| **PWA Manifest** | 应用配置 | ✅ maskable icons, display_override |
...
| **低性能设备** | 高帧率动画导致卡顿 | visibilitychange 检测，后台自动暂停 rAF |
...
| 首屏加载 (LCP) | < 1s |
| 同时跳动计数器 | 17 个独立计算，纯数学无额外 API 调用 |
